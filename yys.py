...
def get_windows(window_name = u"阴阳师-网易游戏"):
	windows = []

	def callback(hwnd, extra):
		if win32gui.GetWindowText(hwnd).decode('gbk') == window_name:
			windows.append(hwnd)
...
windows.sort()
if len(windows) > 1:
	print(u'检测到%s个游戏 请选择要加载的' % len(windows))
	for i, hWnd in enumerate(windows):
		print(i, hWnd)
	idx = input('请输入编号:'.decode('utf8').encode('gbk'))
	hWnd = windows[idx]
	ScreenShotIdx = idx
else:
	ScreenShotIdx = 9
...
def screenshot():
	...
	im_PIL.save("screen%s.jpg"%ScreenShotIdx, 'jpeg')
...
def yuhun():
	while True :
		...
		screenshot()
		screen = cv2.imread('screen%s.jpg'%ScreenShotIdx)
		...
		for i in ['tianzhan11','gu','shuju', 'xuanshang']:#魂11队长
		#for i in ['gu','shuju', 'xuanshang', 'jixu1']:#队员刷
			want = imgs[i]
			size = want[0].shape
			h, w , ___ = size
			target = screen
